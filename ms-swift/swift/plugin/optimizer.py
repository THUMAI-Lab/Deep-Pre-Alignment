...
def get_param_startswith(model,
    ...
    for n, p in model.named_parameters():
        if not p.requires_grad:
            # print('------------------------------------------------', flush=True)
            # print(f'{n} not requires_grad', flush=True)
            continue
        is_rejected = False
        ...
        for prefix in chosen_prefix:
            if n.startswith(prefix):
                ...
                res.append((n, p))
                print(f'------------------------------------------------', flush=True)
                print(f'{n} requires_grad, prefix is {prefix}', flush=True)
                break
    # exit()
    return res
...
def create_multimodal_optimizer(args: 'TrainingArguments', model, dataset):
    ...
    print(f'vit_parameters:{len(vit_parameters)}, aligner_parameters:{len(aligner_parameters)}, llm_parameters:{len(llm_parameters)}', flush=True)
    optimizer_grouped_parameters = []
    for lr, parameters in zip([args.vit_lr, args.aligner_lr, args.learning_rate],
        ...
        for use_wd, wd in zip([False, True], [0., args.weight_decay]):
            ...
            print(f'{type(params)} {len(params)}, wd:{wd}, lr:{lr}', flush=True)
    # exit()
