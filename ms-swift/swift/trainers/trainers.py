...
class Seq2SeqTrainer(SwiftMixin, DataLoaderMixin, HfSeq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if labels is None:
            ...
            # print(f'S2S Trainer Rank-{self.accelerator.process_index} in compute_loss, outputs.loss: {outputs.loss.item():.4f}, num_items_in_batch: {num_items_in_batch}', flush=True)
            
            # fix https://github.com/huggingface/transformers/issues/34263
            if num_items_in_batch is not None:
                outputs.loss = outputs.loss * ((labels[:, 1:] != -100).sum() / num_items_in_batch)

            # print(f'S2S Trainer Rank-{self.accelerator.process_index} in compute_loss, after re-weight outputs.loss: {outputs.loss.item():.4f}, num_items_in_batch: {num_items_in_batch}', flush=True)
        ...
        if getattr(self.args, 'average_tokens_across_devices',
            ...
            print(f'Average tokens across devices: {self.accelerator.num_processes}, num_items_in_batch: {num_items_in_batch}', flush=True)
        else:
            print(f'Not average tokens across devices: {self.accelerator.num_processes}, num_items_in_batch: {num_items_in_batch}', flush=True)
        ...
        # print(f'S2S Trainer Rank-{self.accelerator.process_index} returned loss in compute_loss is {loss.item():.4f}', flush=True)
        return (loss, outputs) if return_outputs else loss

    def training_step(self, model, inputs, *args, **kwargs):
        # Skip batches if ranks have inconsistent pixel_values (some have images, some don't)
        # This avoids DeepSpeed ZeRO-3 gradient sync issues
        if dist.is_initialized() and self.is_in_train:
            rank = dist.get_rank()
            world_size = dist.get_world_size()
            
            # Check if this rank's batch has images
            has_images_local = inputs.get('pixel_values') is not None
            
            # Log local status
            # print(f"[Rank {rank}] Local batch has_images={has_images_local}", flush=True)
            
            # Create tensor for all_reduce: 1 if has images, 0 if no images
            has_images_tensor = torch.tensor([1 if has_images_local else 0], 
                                            device=self.args.device, dtype=torch.long)
            # Sum across all ranks
            dist.all_reduce(has_images_tensor, op=dist.ReduceOp.SUM)
            sum_val = has_images_tensor.item()
            
            # Log aggregated status
            # print(f"[Rank {rank}] After all_reduce: sum={sum_val}, world_size={world_size}", flush=True)
            
            # Skip if inconsistent: not all 0 (all text) and not all world_size (all images)
            if sum_val != 0 and sum_val != world_size:
                logger.warning(f"[Rank {rank}] ⚠️  SKIPPING BATCH - inconsistent pixel_values across ranks ({sum_val}/{world_size} have images)")
                # print(f"[Rank {rank}] ⚠️  SKIPPING BATCH - inconsistent ({sum_val}/{world_size})", flush=True)
                # Return zero loss - all ranks skip together
                return torch.tensor(0.0, device=self.args.device)
            else:
                # Log when proceeding
                status = "all have images" if sum_val == world_size else "all text-only"
                # print(f"[Rank {rank}] ✓ Proceeding with batch - {status}", flush=True)
