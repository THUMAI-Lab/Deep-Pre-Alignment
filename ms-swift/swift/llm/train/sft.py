...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _prepare_template(self) -> None:
        ...
        logger.info(f"Current template_type: {args.template}")
        logger.info(f"Template class: {template.__class__.__name__}")
        logger.info(f"Template meta template_type: {template.template_meta.template_type}")
    ...
    def _post_process_datasets(self, datasets: List) -> List:
        ...
        for i, dataset in enumerate(datasets):
            ...
            if i == 1 and predict_with_generate:
                # val_dataset
                continue
            # Skip LazyLLMDataset when packing is enabled - use pre-encoded cached data directly
            if (args.model_meta.is_multimodal or args.lazy_tokenize) and not args.streaming and not args.packing:
