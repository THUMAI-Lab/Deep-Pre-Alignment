...
class EncodePreprocessor(RowPreprocessor):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        import json
        # print(f"[EncodePreprocessor.preprocess] START - row keys: {list(row.keys())}", flush=True)
        encoded = self.template.encode(row, return_length=True)
        # print(f"[EncodePreprocessor.preprocess] encoded keys: {list(encoded.keys())}, 'length' in encoded: {'length' in encoded}", flush=True)
        # if 'length' in encoded:
            # print(f"[EncodePreprocessor.preprocess] length value: {encoded['length']}", flush=True)
        if self.pre_tokenize:
            # For packing mode, preserve all encoded fields (including multimodal data)
            # For non-packing mode, only keep length for lazy tokenization
            padding_free = getattr(self.template, 'padding_free', False)
            # print(f"[EncodePreprocessor.preprocess] pre_tokenize={self.pre_tokenize}, padding_free={padding_free}", flush=True)
            if padding_free:
                # print(f"[EncodePreprocessor] Preserving encoded fields for packing mode")
                # Convert image_bound tensor to JSON string for proper Arrow serialization
                # Nested lists like [[[18, 284]]] may not serialize properly in Arrow format
                if 'image_bound' in encoded:
                    image_bound = encoded['image_bound']
                    if image_bound and len(image_bound) > 0:
                        if hasattr(image_bound[0], 'tolist'):
                            image_bound_list = [image_bound[0].tolist()]
                        else:
                            image_bound_list = image_bound
                        # Convert to JSON string for reliable caching
                        encoded['image_bound_json'] = json.dumps(image_bound_list)
                        # print(f"[EncodePreprocessor] Converted image_bound to JSON: {encoded['image_bound_json']}")
                    del encoded['image_bound']  # Remove the original nested list

                # Remove large tensor fields - they will be re-computed during training
                for tensor_field in ['pixel_values', 'tgt_sizes', 'temporal_ids']:
                    encoded.pop(tensor_field, None)
                # print(f"[EncodePreprocessor] Removed tensor fields: pixel_values, tgt_sizes, temporal_ids")

                # Merge encoded fields with original row data
                row.update(encoded)
                # print(f"[EncodePreprocessor.preprocess] RETURN row keys (padding_free): {list(row.keys())}, 'length' in row: {'length' in row}", flush=True)
                return row
            else:
                # print(f"[EncodePreprocessor] Discarding encoded fields, keeping only length")
                row['length'] = encoded['length']
                encoded = row
        # print(f"[EncodePreprocessor.preprocess] RETURN encoded keys: {list(encoded.keys())}, 'length' in encoded: {'length' in encoded}", flush=True)
        return encoded
