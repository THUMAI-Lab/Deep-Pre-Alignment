...
class DatasetLoader:
    ...
    def _load_dataset_path(
        ...
        *,
        ...
        # print('----------------------------------------', flush=True)
        # print(f'kwargs: {kwargs} ext: {ext} columns: {columns}', flush=True)
        # print('----------------------------------------', flush=True)
        if file_type == 'csv':
        ...
        # print('----------------------------------------', flush=True)
        # print(f'After load cols: {dataset}', flush=True)
        ...
        # print('----------------------------------------', flush=True)
        # print(f'After rename cols: {dataset}', flush=True)
        
        # Here is good, col names unchanged, 3K samples

        dataset = dataset_meta.preprocess_func(
            dataset, num_proc=num_proc, load_from_cache_file=load_from_cache_file, strict=strict)

        # Here, key names: features: ['messages', 'images', 'source', 'rejected_messages', 'positive_messages', 'negative_messages', 'objects']
        # 0 samples. Fail.

        # print('----------------------------------------', flush=True)
        # print(f'preprocess_func: {dataset_meta.preprocess_func}')
        # print(f'After preprocess_func: {dataset}', flush=True)

        if remove_unused_columns:
            dataset = RowPreprocessor.remove_useless_columns(dataset)

        # print('----------------------------------------', flush=True)
        # print(f'Return dataset: {dataset}', flush=True)
        # print('----------------------------------------', flush=True)
        ...
        *,
        ...
        if streaming:
            ...
            if split_dataset_ratio == 0:
                ...
                # Avoid duplication in the val_dataset.
                # print(f'train_dataset: {train_dataset}, dataset_sample: {dataset_sample}', flush=True)
                # print(f'val_sample: {max(int(min(len(train_dataset), dataset_sample) * split_dataset_ratio), 1)}', flush=True)
                # exit()
    ...
    def load(
        ...
        *,
        ...
        if dataset_syntax.dataset_type == 'path':
            # print('----------------------------------------', flush=True)
            # print(f'\nDataset load from path: {dataset_syntax} {dataset_syntax.dataset_type}', flush=True)
            # print('----------------------------------------', flush=True)
            # print(f'\nDataset_meta: {dataset_meta} columns: {columns} remove_unused_columns:{remove_unused_columns}', flush=True)
...
def load_dataset(
    ...
    *,
    ...
    for dataset in datasets:
        ...
        # print(f'At {__file__} dataset_syntax: {dataset_syntax}\ndataset_meta:{dataset_meta}\nload_kwargs:{load_kwargs}\nuse_hf:{use_hf}', flush=True)
        # exit()
