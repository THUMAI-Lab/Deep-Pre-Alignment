...
class RowPreprocessor:
    standard_keys = _pair_keys + list(
        chain.from_iterable([f'{prefix}_{k}' for k in _pair_keys]
                            for prefix in ['rejected', 'positive', 'negative'])) + [
                                ...
                                'channel',
                                'margin',
                                'index',  # Add index to preserve it through dataset processing
                            ]
    ...
    def batched_to_rows(batched_row: Dict[str, Any]):
        ...
        # print(f'Batched rows have keys: {keys}', flush=True)
    ...
    def batched_preprocess(self, batched_row: Dict[str, Any], *, strict: bool,
        ...
        # print('----------------------------------------', flush=True)
        # print('Call batched preprocess!!!', flush=True)
        ...
        # print('----------------------------------------', flush=True)
        # 1000 list
        # print(f'Len rows: {len(rows)} {type(rows)} {type(rows[0])} {rows[0].keys()}')
        # print(f'rows[0]: {rows[0]["messages"]} {rows[0]["source"]}')
        # exit()

        new_rows = []
        for row in rows:
            try:
                # print(f'row before preprocess is {row.keys()}', flush=True)
                row = self.preprocess(row)
                # TYT: found row to be dict with keys: ['clean_content', 'image_buffer_list', 'source']
                # print(f'row after preprocess is {type(row)}', flush=True)
                # print(f'row after preprocess is {row.keys()}', flush=True)
                # exit()
                # support [row1, row2, ...]
                if row is None:
                    row = []
    ...
    def __call__(
        ...
        *,
        ...
        # print('----------------------------------------', flush=True)
        # # None None
        # print(f'Call {__file__} {self.__class__} batch_size: {batch_size} self.dataset_sample:{self.dataset_sample}', flush=True)
        ...
        # print('----------------------------------------', flush=True)
        # print(f'A. Inside {__file__} {self.__class__}, dataset: {dataset}', flush=True)
        ...
        dataset = RowPreprocessor.get_features_dataset(dataset)
        # print('----------------------------------------', flush=True)
        # print(f'B. Inside {__file__} {self.__class__}, after getfeat, dataset: {dataset}', flush=True)
        assert 'solution' not in dataset.features
        ...
        # print('----------------------------------------', flush=True)
        # print(f'C. Inside {__file__} {self.__class__}, after rename, dataset: {dataset}', flush=True)

        dataset = self.prepare_dataset(dataset)
        # print('----------------------------------------', flush=True)
        # print(f'D. Inside {__file__} {self.__class__}, after prepare, dataset: {dataset}', flush=True)

        dataset = self._cast_pil_image(dataset)
        # print('----------------------------------------', flush=True)
        # print(f'D. Inside {__file__} {self.__class__}, after pil cast, dataset: {dataset}', flush=True)
        ...
        # print('----------------------------------------', flush=True)
        # print(f'E. After rename_columnst, dataset: {dataset}', flush=True)
            ...
            try:
                if isinstance(dataset, HfDataset) and not dataset.cache_files:
                    map_kwargs['cache_file_name'] = os.path.join(get_cache_dir(), 'datasets', 'map_cache',
                                                                 f'{dataset._fingerprint}.arrow')
                # print('----------------------------------------', flush=True)
                # # remove_columns: ['messages', 'images', 'source'] strict:False
                # print(f'F. Before dataset.map, dataset: {dataset} remove_columns: {list(dataset.features.keys())} strict:{strict}', flush=True)
                dataset_mapped = dataset.map(
                    ...
                    **map_kwargs)
                # print('----------------------------------------', flush=True)
                # print(f'G. After dataset.map, dataset.cache_files: {dataset.cache_files} mapconfig:{map_kwargs} dataset: {dataset}', flush=True)
...
class ResponsePreprocessor(RowPreprocessor):
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        # print('----------------------------------------', flush=True)
        # print('Call Response Preprocessor preprocess!!!', flush=True)
        # exit()
...
class MessagesPreprocessor(RowPreprocessor):
    ...
    def __init__(
            ...
            *,
            ...
            keep_first_turn_only: bool = False,
            **kwargs):
        super().__init__(columns=columns, **kwargs)
        ...
        self.keep_first_turn_only = keep_first_turn_only
    ...
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        # print(f'In MessagePreprocessor, row: {row.keys()}', flush=True)
        # print(f'row.message: {type(row["messages"])}{row["messages"]}', flush=True)
        ...
        if isinstance(messages, (str, )):
            ...
            messages = re.sub(r"<image>[^<]*?</image>", "<image>", messages)
            messages = json.loads(messages)['text']
            # <class 'list'> 1 <class 'dict'> dict_keys(['buffer', 'image_id']) <class 'bytes'> 03d2e6a2acf03f1e02966854462eb041 isinstance(image, BytesIO): False
            # print(f'Images type: {type(row["images"])} {len(row["images"])} {type(row["images"][0])} {row["images"][0].keys()} {type(row["images"][0]["buffer"])} {row["images"][0]["image_id"]} isinstance(image, BytesIO): {isinstance(row["images"][0]["buffer"], BytesIO)}', flush=True)
            row['images'] = [img_obj['buffer'] for img_obj in row['images']]
            # exit()
        ...
        # Optionally keep only the first conversation turn (system + first user + first assistant)
        def _env_true(v: Optional[str]) -> bool:
            return str(v).lower() in {'1', 'true', 'yes', 'y', 'on'}
        if self.keep_first_turn_only or _env_true(os.environ.get('KEEP_FIRST_TURN_ONLY', 'False')):
            old_size = len(messages)
            # print(f'Keep first turn only', flush=True)
            first_turn: List[Dict[str, str]] = []
            idx = 0
            # Preserve initial system message if present
            if messages and messages[0].get('role') == 'system':
                first_turn.append(messages[0])
                idx = 1
            user_added = False
            assistant_added = False
            while idx < len(messages) and not assistant_added:
                role = messages[idx].get('role')
                if not user_added and role == 'user':
                    first_turn.append(messages[idx])
                    user_added = True
                elif user_added and role == 'assistant':
                    first_turn.append(messages[idx])
                    assistant_added = True
                    break
                idx += 1
            # Fallbacks: if we didn't capture anything meaningful, keep at least the first message
            if not first_turn and messages:
                first_turn = [messages[0]]
            messages = first_turn
            # print(f'Old size: {old_size}, New size: {len(messages)}', flush=True)
...
class AutoPreprocessor:
    ...
    def __call__(
        ...
        *,
        ...
        # print('----------------------------------------', flush=True)
        # print(f'Inside {__file__}, dataset: {dataset}', flush=True)
        dataset = RowPreprocessor.safe_rename_columns(dataset, self.columns)
        # print('----------------------------------------', flush=True)
        # print(f'Inside {__file__}, after rename, dataset: {dataset}', flush=True)
        preprocessor = self._get_preprocessor(dataset)
        # print('----------------------------------------', flush=True)
        # print(f'Inside {__file__}, preprocessor: {preprocessor}', flush=True)
        dataset = preprocessor(dataset, num_proc=num_proc, load_from_cache_file=load_from_cache_file, strict=strict)
        # print('----------------------------------------', flush=True)
        # print(f'Inside {__file__}, return: {dataset}', flush=True)
        return dataset
