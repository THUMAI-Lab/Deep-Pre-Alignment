...
class TunerArguments:
    ...
    def _init_multimodal_full(self):
        print(f'{__file__} {__class__.__name__}, {self.freeze_llm}, {self.freeze_vit}, {self.freeze_aligner}', flush=True)
        model_arch = self.model_meta.model_arch
        print(f'model_arch: {model_arch} model_meta: {self.model_meta}, train_type: {self.train_type}', flush=True)
        # exit()
        ...
        print(f'freeze_parameters: {self.freeze_parameters}', flush=True)
        print(f'trainable_parameters: {self.trainable_parameters}', flush=True)
