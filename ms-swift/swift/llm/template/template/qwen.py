...
class Qwen2VLTemplate(Template):
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif hasattr(base_model.model, "language_model"):
            inputs_embeds = base_model.model.language_model.embed_tokens(input_ids)
        elif hasattr(base_model.model, "vlm"):
            inputs_embeds = base_model.model.vlm.get_input_embeddings()(input_ids)
        else:
            inputs_embeds = base_model.model.thinker.model.embed_tokens(input_ids)
        if hasattr(base_model, "visual"):
            visual_module = base_model.visual
        else:
            visual_module = base_model.model.perceiver.visual
        inputs_embeds = self._get_inputs_embeds_hf(inputs_embeds, inputs, visual_module, self.processor, model.config)
    ...
    def _get_position_ids(self, inputs: Dict[str, Any]):
        ...
        elif hasattr(base_model.model, "get_rope_index"):
            get_rope_index = base_model.model.get_rope_index
        elif hasattr(base_model.model, "vlm") and hasattr(base_model.model.vlm, "get_rope_index"):
            get_rope_index = base_model.model.vlm.get_rope_index
        elif hasattr(base_model.model, "perceiver") and hasattr(base_model.model.perceiver.model, 'vlm'):
            get_rope_index = base_model.model.perceiver.model.vlm.get_rope_index
        else:
            get_rope_index = base_model.model.perceiver.model.get_rope_index
...
class ForwardVisualTokensArchTemplate(Qwen2_5VLTemplate):
    _thinker_tokenizer = None
    _thinker_template = None
    trim_perceiver_after_image: bool = False
    support_padding_free = True

    @property
    def thinker_tokenizer(self):
        from ...model.register import get_model_tokenizer
        if self._thinker_tokenizer is None:
            _, self._thinker_tokenizer = get_model_tokenizer('Qwen/Qwen3-4B', load_model=False)
        return self._thinker_tokenizer

    @property
    def thinker_template(self):
        if self._thinker_template is None:
            # Import get_template here to avoid circular import
            from ..register import get_template

            self._thinker_template = get_template(
                template_type='qwen3',
                processor=self.thinker_tokenizer,
                max_length=32768,
                default_system=self.template_meta.default_system
            )
            self._thinker_template.replace_tag = self.replace_tag
        self._thinker_template.set_mode(self.mode)
        return self._thinker_template

    def init_env_args(self):
        super().init_env_args()
        # Enable trimming perceiver tokens after image content to save memory
        # Usage: export trim_perceiver_after_image=true
        self.trim_perceiver_after_image = get_env_args('TRIME_PERCEIVER_AFTER_IMAGE', bool, False)

    def packing_row(self, row: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Pack multiple samples into a single sequence for efficient training.

        For duplex architecture, we need position_ids for BOTH:
        1. Thinker's packed input_ids
        2. Perceiver's packed input_ids_of_perceiver

        Both need position_ids to distinguish sequence boundaries in flash attention.
        """
        # Get position_ids for thinker
        position_ids = []
        # Get position_ids for perceiver
        position_ids_of_perceiver = []
        # Collect indices from all packed samples
        indices = []

        for r in row:
            r = r.copy()

            # Collect index if available
            if 'index' in r:
                indices.append(r['index'])

            # Position IDs for thinker
            r['input_ids'] = torch.tensor(r['input_ids'])[None]
            position_ids.append(self._get_position_ids(r))

            # Position IDs for perceiver
            if 'input_ids_of_perceiver' in r:
                r_perceiver = r.copy()
                r_perceiver['input_ids'] = torch.tensor(r['input_ids_of_perceiver'])[None]
                position_ids_of_perceiver.append(self._get_position_ids(r_perceiver))

        # Parent will pack input_ids, labels, loss_scale, AND input_ids_of_perceiver
        packed = super().packing_row(row)

        # Add position_ids for both thinker and perceiver
        packed['position_ids'] = torch.concat(position_ids, dim=-1)
        if position_ids_of_perceiver:
            packed['position_ids_of_perceiver'] = torch.concat(position_ids_of_perceiver, dim=-1)

        # Store list of indices for this packed sequence
        if indices:
            packed['index'] = indices

        return packed

    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        # print(f'Template is:{self}', flush=True)

        self.max_pixels = get_env_args('MAX_PIXELS', int, None)
        # print(f'--------------------------------', flush=True)
        # print(f'self.max_pixel: {self.max_pixels}', flush=True)
        # print(f'self.max_length: {self.max_length}', flush=True)
        # print(f'--------------------------------', flush=True)
        if self.max_pixels is not None:
            self.processor.image_processor.max_pixels = self.max_pixels
            self.processor.video_processor.max_pixels = self.max_pixels

        # print("--------------------------------")
        # print(f"inputs.messages: {inputs.messages}", flush=True)
        # print("--------------------------------")
        encoded = super()._encode(inputs)
        processor = self.processor
        input_ids_of_perceiver = encoded["input_ids"]
        del encoded["labels"]

        loss_scale = encoded.get("loss_scale", None)
        # print("--------------------------------")
        # print(
        #     f"input_ids_of_perceiver: {input_ids_of_perceiver} {self.processor.tokenizer.convert_ids_to_tokens(input_ids_of_perceiver)}\n{self.processor.tokenizer.decode(input_ids_of_perceiver)}", flush=True
        # )
        # exit()
        # print("--------------------------------")
        # print(f"Thinker tokenizer: {self.thinker_tokenizer}", flush=True)
        # print("--------------------------------")

        thinker_encoded = self.thinker_template._encode(inputs)
        input_ids = thinker_encoded['input_ids']
        labels = thinker_encoded['labels']
        # print("--------------------------------")
        # print(f"input_ids: {self.thinker_tokenizer.convert_ids_to_tokens(input_ids)}\n{self.thinker_tokenizer.decode(input_ids)}", flush=True)
        # print("--------------------------------")

        # replace -100 with pad token id
        # labels_to_print = [0 if label == -100 else label for label in labels]
        # print(f"labels: {self.thinker_tokenizer.convert_ids_to_tokens(labels_to_print)}\n{self.thinker_tokenizer.decode(labels_to_print)}", flush=True)
        # print(f"labels: {labels}", flush=True)
        # print("--------------------------------")

        # print(f'media_type is {inputs.media_type}', flush=True)
        for media_type in ["images", "videos"]:
            mm_data = getattr(inputs, media_type)
            if mm_data:
                # print(f'mm_data is {mm_data}, media_type is {media_type}', flush=True)
                if media_type == "images":
                    media_token = self.image_token_id
                    ...
                    raise NotImplementedError("Video is not supported for perceiver")
                idx_list = findall(input_ids, media_token)
                merge_length = processor.image_processor.merge_size**2
                ...
                def _get_new_tokens(i):
                    if media_type == "images":
                        ...
                        raise NotImplementedError(
                            "Video is not supported for perceiver"
                        )
                # print(f'{idx_list} _get_new_tokens: {_get_new_tokens(0)}', flush=True)
                input_ids, labels, loss_scale = self._extend_tokens(
                    input_ids, labels, loss_scale, idx_list, _get_new_tokens
                )
                # print(f'Image processor: {processor.image_processor}', flush=True)
                encoded.update(media_inputs)

        # Optionally trim perceiver tokens to keep only up to the end of image content (inclusive)
        if self.trim_perceiver_after_image:
            # print(f'Trim perceiver after image', flush=True)
            # Identify the first occurrence of the vision end token in perceiver sequence
            vision_end_ids = self._tokenize('<|vision_end|>')
            assert len(vision_end_ids) == 1, "Expected exactly one vision end token"
            if vision_end_ids:
                end_id = vision_end_ids[0]
                # Ensure we operate on a Python list
                if not isinstance(input_ids_of_perceiver, list):
                    input_ids_of_perceiver = input_ids_of_perceiver.tolist()
                if end_id in input_ids_of_perceiver:
                    end_idx = input_ids_of_perceiver.index(end_id)
                    input_ids_of_perceiver = input_ids_of_perceiver[: end_idx + 1]

        # Store input_ids_of_perceiver and generate corresponding attention mask
        # During training, this will be batched by the DataLoader, so we store single sequences here
        encoded["input_ids_of_perceiver"] = input_ids_of_perceiver

        # assert len(input_ids_of_perceiver) == len(input_ids), f"input_ids_of_perceiver: {len(input_ids_of_perceiver)}, input_ids: {len(input_ids)}"

        # Generate attention mask - all positions are attended to (no padding in single sample)
        if isinstance(input_ids_of_perceiver, list):
            encoded["attention_mask_of_perceiver"] = [1] * len(input_ids_of_perceiver)
        else:
            # Handle tensor case if needed
            encoded["attention_mask_of_perceiver"] = [1] * len(input_ids_of_perceiver)
        ...
        # find all 151667, 271, 151668, 271 sub sequence in labels and replace with -100 -100 -100 -100
        for i in range(len(labels) - 3):
            if labels[i:i+4] == [151667, 271, 151668, 271]:
                labels[i:i+4] = [-100, -100, -100, -100]
        encoded["labels"] = labels
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        # Ensure custom parameters are preserved for the duplex model
        result = inputs.copy()

        # These parameters are required by ForwardVisualTokensArchModel.forward()
        # but are not in the default whitelist in base.py:1341-1343
        perceiver_keys = [
            'input_ids_of_perceiver',
            'attention_mask_of_perceiver',
            'position_ids_of_perceiver',
            'index'  # Add index for tracking which sample is being processed
        ]
    ...
    def _data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        """
        Custom data collator following Qwen2VLTemplate pattern.

        We need to ensure input_ids_of_perceiver, attention_mask_of_perceiver,
        and position_ids_of_perceiver are properly handled since they're not in the parent's default key list.
        """
        # Call parent first to handle standard fields
        res = super()._data_collator(batch, padding_to=padding_to)

        # Handle perceiver-specific fields explicitly (parent doesn't know about these)
        if self.padding_free:
            # Packing mode: padding_free calls packing_row, which puts everything in batch[0]
            for key in ['input_ids_of_perceiver', 'attention_mask_of_perceiver', 'position_ids_of_perceiver']:
                if key in batch[0]:
                    res[key] = [batch[0][key]]
        else:
            # Non-packing mode: collect from all batch elements
            for key in ['input_ids_of_perceiver', 'attention_mask_of_perceiver', 'position_ids_of_perceiver']:
                values = [b[key] for b in batch if b.get(key) is not None]
                if values:
                    res[key] = values

        # Now convert to tensors and pad (following parent's pattern)
        for key in ['input_ids_of_perceiver', 'attention_mask_of_perceiver', 'position_ids_of_perceiver']:
            if key not in res:
                continue

            # Determine pad value
            if key == 'input_ids_of_perceiver':
            ...
            # Pad sequences
            if len(res[key]) > 1 or (len(res[key]) == 1 and not self.padding_free):
                ...
                # Single sequence in packing mode
                res[key] = res[key][0].unsqueeze(0) if res[key][0].ndim == 1 else res[key][0]

        # Handle position_ids for thinker (standard Qwen2-VL logic)
        if not self.padding_free and self.is_training:
            res['position_ids'] = self._get_position_ids(res)
        if 'position_ids' in res:
            position_ids = res['position_ids']
        ...
        # Handle position_ids for perceiver (similar logic)
        if 'position_ids_of_perceiver' in res:
            position_ids_perceiver = res['position_ids_of_perceiver']
            # Extract text_position_ids for perceiver (for flash attention)
            if position_ids_perceiver.ndim == 2 and position_ids_perceiver.shape[0] == 1:
                # Packing mode with single batch element
                res['text_position_ids_of_perceiver'] = position_ids_perceiver[0]
                # Get packed params for perceiver's flash attention
                if self.transformers_version >= version.parse('4.53.0.dev'):
                    perceiver_params = get_packed_seq_params(position_ids_perceiver)
                    # Rename keys to avoid conflict with thinker's params
                    res['cu_seq_lens_q_perceiver'] = perceiver_params['cu_seq_lens_q']
                    res['cu_seq_lens_k_perceiver'] = perceiver_params['cu_seq_lens_k']
                    res['max_length_q_perceiver'] = perceiver_params['max_length_q']
                    res['max_length_k_perceiver'] = perceiver_params['max_length_k']

        # Handle 'index' field - collect indices from batch
        if self.padding_free:
            # Packing mode: batch[0] contains packed data with list of indices
            if 'index' in batch[0]:
                res['index'] = batch[0]['index']
        else:
            # Non-packing mode: collect indices from all batch elements
            indices = [b.get('index') for b in batch if b.get('index') is not None]
            if indices:
                # Convert to tensor for batched processing
                res['index'] = torch.tensor(indices, dtype=torch.long)
...
register_template(
    QwenTemplateMeta(
        MLLMTemplateType.duplex_forward_visual_tokens_arch,
        template_cls=ForwardVisualTokensArchTemplate,
        default_system=None,
    )
)
