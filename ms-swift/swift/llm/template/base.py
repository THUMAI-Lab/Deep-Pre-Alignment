...
class Template(ProcessorMixin):
    ...
    def packing_row(self, row: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for key in keys:
            if key in {'input_ids', 'labels', 'loss_scale', 'input_ids_of_perceiver'}:
    ...
    def _pre_tokenize_images(self, context_list: List[Context], loss_scale_list: List[float],
        ...
        # print(f"context_list in _pre_tokenize_images: {context_list}", flush=True)
        # print(f"{self.__class__.__name__} call _pre_tokenize_images", flush=True)
        for context, loss_scale in zip(context_list, loss_scale_list):
    ...
    def _swift_encode(self, inputs: StdTemplateInputs):
        # TODO: Core Encode Code Here
        template_meta = self.template_meta
        # print(f'template_meta: {template_meta}', flush=True)
        # print(f'inputs before prepare: {inputs}', flush=True)
        self._swift_prepare_inputs(inputs)
        # print(f'inputs after prepare: {inputs}', flush=True)
        system = self._get_system(inputs)

        # print(f'system: {system}', flush=True)
        # print(f'inputs before get_std_messages: {inputs}', flush=True)
        self._get_std_messages(inputs.messages)
        # print(f'inputs after get_std_messages: {inputs}', flush=True)
        ...
        if self.is_training:
            ...
            answer_len = 0
        # print(f'res_context_list: {res_context_list}', flush=True)
        # print(f'loss_scale_list: {loss_scale_list}', flush=True)
        # print(f'answer_len: {answer_len}', flush=True)
        # exit()
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        if self.is_encoder_decoder or self.mode == 'gkd':
            ...
            # print(f"{self.__class__.__name__}, labels in base.py Template._encode: ", labels, flush=True)
        ...
        if not self.is_training:
            for k in list(encoded.keys()):
                if k.endswith('labels') or k.endswith('loss_scale'):
                    encoded[k] = None
        # print(f'{self.mode}, {self.__class__.__name__}, encoded in base.py Template._encode["labels"]: ', encoded['labels'], flush=True)
        return encoded
    ...
    def set_mode(self, mode: Literal['pt', 'vllm', 'lmdeploy', 'sglang', 'train', 'rlhf', 'kto', 'gkd']) -> None:
        # print(f'set mode from {self.mode} to {mode} in {self.__class__.__name__}', flush=True)
        self.mode = mode
